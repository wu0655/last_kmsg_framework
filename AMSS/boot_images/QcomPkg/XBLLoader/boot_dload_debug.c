...
void dload_mem_debug_init(void)
{
  ...
  */
  ...
  ver_prop = smem_get_addr(SMEM_ID_VENDOR1,(uint32*)&len);
  /* Add Version Info */
  if(len!=0 && ver_prop!=NULL)
  {
  n = qsnprintf(s, POS(bufsize), ";ro.build.type: %s\n", ver_prop);
  }
  else
  {
  n = qsnprintf(s, POS(bufsize), ";ro.build.type: %s\n", "No version Info can be found");
  }
  smem_free(ver_prop);
  s += n;
  bufsize -= n;

  ver_prop = smem_get_addr(SMEM_ID_VENDOR2,(uint32*)&len);
  /* Add Version Info */
  if(len!=0 && ver_prop!=NULL)
  {
  n = qsnprintf(s, POS(bufsize), ";ro.build.id: %s\n", ver_prop);
  }
  else
  {
  n = qsnprintf(s, POS(bufsize), ";ro.build.id: %s\n", "No version Info can be found");
