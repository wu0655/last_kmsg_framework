...
void do_prk_utc_cali(const void *ts, int source)
{
	u64 local_in_nsec;
	u64 utc_in_usec;
	struct timespec *p_ts = (struct timespec *)ts;
	...
	/* utc time is ns */
	utc_in_usec = p_ts->tv_nsec + p_ts->tv_sec * NSEC_PER_SEC;

	prk_utc_cali = (utc_in_usec > local_in_nsec) ? (utc_in_usec - local_in_nsec) : 0;
	return;
}
...
static size_t print_time_utc(u64 ts, u64 utc_nsec, char *buf)
{
	...
	t_ns = ts + utc_nsec;
	do_div(t_ns, NSEC_PER_SEC);
	rtc_time_to_tm(t_ns, &tm);
	check_tm(&tm);

	/* local time */
	rem_usec = do_div(ts, NSEC_PER_SEC);
...
static size_t print_prefix(const struct printk_log *msg, bool syslog, char *buf)
{
	...
	len += print_time_utc(msg->ts_nsec, msg->utc_nsec, buf ? buf + len : NULL);
	return len;
}
...
#ifdef CONFIG_RAM_LOG
unsigned long get_prk_utc_cali(void)
{
	return prk_utc_cali;
}
#endif
